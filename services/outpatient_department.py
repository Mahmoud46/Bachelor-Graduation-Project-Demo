...
class OutpatientDepartment:
    ...
    def get_summary_report(self,main_analysis):
        ...
        summary_report+=f"<ul><li><h3>Patient number</h3> <p>The <i>total paient number</i> in the outpatient department is " 
        ...
        summary_report+=f"<b>{self.get_min_clinic_in(main_analysis['mean_waiting_times'],main_analysis['clinic_types'])}</b> has the <i>minimum waiting time</i> with <b>{round(min(main_analysis['mean_waiting_times']))}</b> minutes with arrival rate of <b>{round(main_analysis['mean_arrival_rate_hrs'][self.get_min_index_clinic_in(main_analysis['mean_waiting_times'])])}</b> patient per hour</p> </li> <li> <h3>Service time for patient</h3> <p>The <i>total service time</i> in the outpatient department is {round(sum(main_analysis['mean_service_time']))} minutes with <i>average</i> of {round(np.average(main_analysis['mean_service_time']))} minutes, and we found that "
